...
//END CONFIG AREA ----------------------------------------------------------
if(startSession() && session_read('admin-red')!="" && session_read('admin-red') != 'admin_logout.php')
{//store redirect to get directly back to originating page
	$admin_red = session_read('admin-red');
...
if(session_read('admin-red') !="" && session_read('admin-red') == 'admin_logout.php')
{#since admin_logout.php uses the session var to pass feedback, kill the session here!
	session_eliminate('admin-red');
}
session_close();
?>
