...
{//if POST is set, prepare to process form data
	...
	{# valid user, create session vars, redirect!
		...
		session_write('AdminID',$AdminID) ; # create session variables to identify admin
		session_write("FirstName",dbOut($row["FirstName"]));  #use dbOut() to clean strings, replace escaped quotes
		session_write("Privilege", dbOut($row["Privilege"]));
		...
		if(session_read('admin-red') != "")
		{#check to see if we'll be redirecting to a requesting page
			$red = session_read('admin-red'); #redirect back to original page
			session_write('admin-red', ''); #clear session var
