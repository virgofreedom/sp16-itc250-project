...
{//Select administrator
	...
	if(session_read('Privilege') == "admin")
	...
	if(session_read('Privilege') == "developer" || session_read('Privilege') == "superadmin")
	...
	{# limit access to the individual, if not developer level
		$sql .= " where AdminID=" . $session_read("AdminID");
	}
...
function editDisplay()
{
	...
	';
		...
		{# uses createSelect() function to preload the select option
			...
			echo '<input type="hidden" name="Privilege" value="' . session_read('Privilege') . '" />';
		}	
	echo '
...
function updateExecute()
{
	...
	if (mysqli_affected_rows($iConn) > 0){
	 ...
	 if(session_read("AdminID") == $AdminID)
	 {#this is me!  update current session info:
		session_write('Privilege', $Privilege);
	 	session_write("FirstName",$FirstName);
	 }
	}else{
